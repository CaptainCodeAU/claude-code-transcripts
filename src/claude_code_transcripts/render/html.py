"""Full-session HTML generation and batch archiving."""
...
from .blocks import make_msg_id, render_message
from .env import get_macros, get_template
from .indexes import _generate_master_index, _generate_project_index
...
def generate_batch_html(
    ...
    for project in projects:
        ...
        for session in project["sessions"]:
            ...
            # Call via the package namespace so tests that patch
            # ``claude_code_transcripts.generate_html`` still intercept it.
            try:
                _root.generate_html(session["path"], session_dir)
                if include_json:
...
def generate_html(json_path, output_dir, github_repo=None):
    ...
    for i, conv in enumerate(conversations):
        ...
        for lt in stats["long_texts"]:
            ...
            long_texts_html += get_macros().index_long_text(rendered_lt, lt)

        stats_html = get_macros().index_stats(tool_stats_str, long_texts_html)

        item_html = get_macros().index_item(
            prompt_num,
            link,
            conv["timestamp"],
            rendered_content,
